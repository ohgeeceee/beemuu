...
const DECODERS = {
  ...
  // v0.19+ expansions (best-effort from community traces)
  0x2C4: { name: "intake_air", decode: (f) => ({ intakeTemp: (f[1] || 0) - 40 }) },
  0x1A0: { name: "engine_load", decode: (f) => ({ load: ((f[2] || 0) * 100) / 255 }) },
  0x3B8: { name: "cruise_status", decode: (f) => ({ cruiseActive: !!(f[0] & 0x08) }) },
  0x0F4: { name: "fuel_rail", decode: (f) => ({ fuelRail_kPa: ((f[0] || 0) << 8 | (f[1] || 0)) * 10 }) },
  // additional safe E/F series (best-effort scales)
  0x1D1: { name: "manifold", decode: (f) => ({ map_kPa: ((f[0] || 0) << 8 | (f[1] || 0)) / 10 }) },
  0x2D0: { name: "oil_pressure", decode: (f) => ({ oilPress_bar: ((f[1] || 0) * 0.05) }) },
  0x3E0: { name: "ambient_ext", decode: (f) => ({ extTemp: (f[0] || 0) - 40 }) },
  // more for v0.19+
  0x2C0: { name: "intake_detail", decode: (f) => ({ iat: (f[1]||0)-40, map: ((f[2]||0)<<8|(f[3]||0))/10 }) },
  0x0D1: { name: "torque_detail", decode: (f) => ({ torqueNm: (f[0]||0)*0.5, throttle: (f[2]||0)*0.4 }) },
  0x3D0: { name: "status_bits", decode: (f) => ({ acOn: !!(f[0]&0x01), fan: (f[1]||0)&0x03 }) },
  // additional safe decoders
  0x2C2: { name: "coolant_detail", decode: (f) => ({ coolant2: ((f[0]||0)-48) }) },
  0x1D2: { name: "ambient2", decode: (f) => ({ amb: (f[0]||0)-40 }) },
  0x0B4: { name: "wheel_status", decode: (f) => ({ absActive: !!(f[0]&0x04) }) },
  // more
  0x3A0: { name: "ac_status", decode: (f) => ({ acRequested: !!(f[0] & 0x80), blower: (f[1]||0) & 0x0F }) },
  0x2D1: { name: "temp_detail", decode: (f) => ({ oilTemp2: ((f[0]||0) - 48) }) },
  // v0.20 additional
