...
>
> **Audit date:** 2026-07-29, against `main` @ `0342efe`
> (post-`#182` community doc fixes).
>
...
The audit ran a repo-wide search for `NOT YET IMPLEMENTED`,
`migration in progress`, `planned v0`, and `v0.6.0`. The four
false claims that this proposal addresses:
...
| `CLAUDE.md` | 154-155 | "No new raw VIN DID reads (the `protocol::read_vin` router is a tracked v0.6.0 issue)." | **DONE.** `src-tauri/src/protocol/mod.rs::read_vin` (UDS 22 F190 + KWP 1A 90 + CAS fallback) is on main and called from `src-tauri/src/commands.rs::read_vehicle_info:454`. |
| `.claude/agents/fix-drafter.md` | 53 | "(…); the `protocol::read_vin` router is a planned v0.6.0 fix." | **DONE** (same as above). The `read_vin` router is shipped. |
